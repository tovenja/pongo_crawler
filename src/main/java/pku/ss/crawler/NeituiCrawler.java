 ...
 */
@Service
public class NeituiCrawler {
    public static final String CRAWLED_DATA = "/data/neitui/crawled.dat";
    public static final String FAILED_DATA = "/data/neitui/failed.dat";
    private static ExecutorService es = Executors.newFixedThreadPool(20);
    static BloomFilter<byte[]> filter = BloomFilter.create(Funnels.byteArrayFunnel(), 35000);
    ...
    public void crawler() throws Exception {
        ...
        for (int i = 600; i < 927/*927*/; i++) {
            NeituiJob job = new NeituiJob(i, filter, failedSet, jobDao);
            es.submit(job);
        }
        es.shutdown();
